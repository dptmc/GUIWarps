	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		for(Iterator iter = Main.ess.getWarps().getList().iterator(); iter.hasNext(); ++page) {
			...
			if (page == 1 && Main.ess.getWarps().getCount() > 54) {
				pageI.getInventory().setItem(53, this.getItemStackWithName(this.config.getString("nextPage"), Material.getMaterial((int)this.config.getInt("GUIWarps.nextID"))));
			} else if (page != 1 && Main.ess.getWarps().getCount() > 53) {
				pageI.getInventory().setItem(53, this.getItemStackWithName(this.config.getString("nextPage"), Material.getMaterial((int)this.config.getInt("GUIWarps.nextID"))));
				pageI.getInventory().setItem(45, this.getItemStackWithName(this.config.getString("prevPage"), Material.getMaterial((int)this.config.getInt("GUIWarps.prevID"))));
			} else if (page != 1 && Main.ess.getWarps().getCount() <= 53) {
				pageI.getInventory().setItem(45, this.getItemStackWithName(this.config.getString("prevPage"), Material.getMaterial((int)this.config.getInt("GUIWarps.nextID"))));
			}
